...
class SaveWayPoints:
    def __init__(self, mission_file_dir,mission_file):
        ...
        '''
        iterator is first being buffered (up to the system default buffer size, which for me is 8096
        generate_geojson_dict is an generator that triggers main_loop. 
        json.dump dumps data to file as it comes. 
        '''
        geojson_dict = self.generate_geojson_dict()
    ...
    @streamable_dict
    def generate_geojson_dict(self):
        yield "type", "LineString"
        yield "coordinates", self.main_loop()
        # main loop will get blocked here, we cannot run another generator to push odometry data. need to find a workaround. 
        # you can do nested operations serially by adding more generators, but not the parallel operations(asyncIO) i.e., writing to coordinates key and odometry key at once. 
        yield "Is_RTK_Good", self.RTK_Status
                ...
                if not self.RTK_fail_status or self.SAVE_ON_RTKLOSS:
                    if self.is_first_point:
                        if self.gps_data_msg and self.odom_data: # and self.imu_data:
                            ...
                            yield [self.gps_data_msg.longitude, self.gps_data_msg.latitude]#, self.odom_data
