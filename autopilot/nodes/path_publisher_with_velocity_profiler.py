...
class PathPubGps:
    def __init__(self, mission_file):
        ...
        i = 0
        lhd = 4
        len_traj = len(trajectory_msg.points)
        for i in range(len_traj):
            if i < 10 or i > len_traj - 10:
                # assign min speed
                pass
            else:
                acc_dis = trajectory_msg.points[i].accumulated_distance_m
                robot_pose = trajectory_msg.points[i].pose
                for inner_ind in range(i, len_traj):
                    if trajectory_msg.points[inner_ind].accumulated_distance_m - acc_dis >= lhd:
                        break
                slope = get_poses_slope(self.trajectory_data.points[inner_ind].pose, robot_pose)
                ...
                if abs(delta_degrees) < 10:
                    trajectory_msg.points[i].longitudinal_velocity_mps = max_forward_speed
                else:
                    trajectory_msg.points[i].longitudinal_velocity_mps = np.interp(steering_angle, 10, 30,
                                                                                   min_forward_speed, max_forward_speed)

        len_traj = len(trajectory_msg.points)
        speed_limit = 0.2
        prev_vel = trajectory_msg.points[0].longitudinal_velocity_mps
        for i, traj_point in trajectory_msg.points:
            vel_diff = prev_vel - traj_point.longitudinal_velocity_mps
            if abs(vel_diff) < speed_limit:
                continue
            elif vel_diff > 2:
                trajectory_msg.points[i-1].longitudinal_velocity_mps = \
                    min(trajectory_msg.points[i-1].longitudinal_velocity_mps, traj_point.longitudinal_velocity_mps - speed_limit
                                                                           )
                for j in range(i-1, 0, -1):
                    if  trajectory_msg.points[j - 1].longitudinal_velocity_mps -  trajectory_msg.points[j].longitudinal_velocity_mps:

                    trajectory_msg.points[j] = min(trajectory_msg.points[j].longitudinal_velocity_mps,
                                                   traj_point.longitudinal_velocity_mps - speed_limit)

                pass
            else:
                # forward search and smooth speed transition
                pass
