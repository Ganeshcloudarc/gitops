...
std::vector<RDP::Point2d> RDP::DouglasPeucker::Simplify(std::vector<Point2d>& pointList, double epsilon)
{
	...
	{
		double d = perpendicularDistance(pointList[i], pointList[0], pointList[pointList.size() - 1]);
		if (d > dmax) {
			index = i;
			dmax = d;
		}
...
double RDP::DouglasPeucker::perpendicularDistance(const Point2d& p, const Point2d& line_p1, const Point2d& line_p2)
{
	Point2d vec1 = Point2d(p.x_ - line_p1.x_, p.y_ - line_p1.y_);
	Point2d vec2 = Point2d(line_p2.x_ - line_p1.x_, line_p2.y_ - line_p1.y_);
	double d_vec2 = sqrt(vec2.x_*vec2.x_ + vec2.y_*vec2.y_);
	double cross_product = vec1.x_*vec2.y_ - vec2.x_*vec1.y_;
	double d = abs(cross_product / d_vec2);
	return d;
}
