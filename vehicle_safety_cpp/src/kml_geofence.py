...
class KmlToGeofence: 
            ...
            with open(kml_file_dir,'r')as geo_fence:
                ...
                for i in k.features(): 
                    ...
                    for j in i.features(): 
                        ...
                        # condn for linestring
                        elif isinstance (j.geometry,geometry.LineString): 
                            polygon = j.geometry 
                            for coordinates in polygon.coords:
                                self.line_string_kml.append([coordinates[1],coordinates[0]])  
                            self.line_string_kml.append(self.line_string_kml[0]) 
                            # since to connect the last and first point and make it as closed geofence
                        else: 
                            rospy.logerr("kml is not of linestring or polygon or may be corrupted")
                ...
                if (self.size_kml != 0):
                    ...
                    rospy.loginfo("geofence params set successfully for geo_fence_coordinates - polygon") 
                    self.geo_fence_cb() 

                # line_string type in kml 
                elif (len(self.line_string_kml) !=0):  
                    self.geofence = self.line_string_kml
                    rospy.set_param('/vehicle_safety/geo_fence_coordinates',self.geofence) 
                    rospy.loginfo("geofence params set successfully for geo_fence_coordinates - linestring")
                    self.geo_fence_cb()
                # when multiple polygon of coordinates are selected
                    ...
                    rospy.loginfo("Geofence params set successful from geofence.kml - multipolygon")
                    self.no_go_zone_cb() 
    ...
    def no_go_zone_cb(self):
            ...
            rospy.loginfo("update no go zone param from geofence.kml")  

    def geo_fence_cb(self): 
            rospy.wait_for_service('geo_fence_coordinates')
            try : 
                update_only_geo_fence = rospy.ServiceProxy('geo_fence_coordinates',trigger) 
                rospy.loginfo("ros client is running - geofence coords") 
                return update_only_geo_fence 
            
            except rospy.ServiceException as e: 
                rospy.loginfo("service call failed %s" %e) 
            rospy.loginfo("update geofence param from geofence.kml") 
