...
class KmlToGeofence: 
    def __init__(self,kml_file_dir): 
        ...
        try:
            ...
            rospy.logerr(f"Error in reading: {self.kml_file_dir}, err : {err}") 
            rospy.logdebug(f"length of kml way points: {len(self.kml_coords)}")
        
        # when only geofence coordinates is selected
        if (self.size_kml != 0):
        ...
        # when multiple polygon of coordinates are selected
            ...
            rospy.loginfo("params are fixed successfully")
        self.cb_clt()
