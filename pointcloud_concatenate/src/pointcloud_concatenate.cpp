...
ros::Time cloud_in1_received_time, cloud_in2_received_time, cloud_in3_received_time, cloud_in4_received_time;
bool is_cloud_1_time_out = false;
bool is_cloud_2_time_out = false;
bool is_cloud_3_time_out = false;
bool is_cloud_4_time_out = false;
...
void PointcloudConcatenate::subCallbackCloudIn1(sensor_msgs::PointCloud2 msg) {
  ...
  cloud_in1_received_time = ros::Time::now();
  is_cloud_1_time_out = false;
}
...
void PointcloudConcatenate::subCallbackCloudIn2(sensor_msgs::PointCloud2 msg) {
  ...
  cloud_in2_received_time = ros::Time::now();
  is_cloud_2_time_out = false;
}
...
void PointcloudConcatenate::subCallbackCloudIn3(sensor_msgs::PointCloud2 msg) {
  ...
  cloud_in3_received_time = ros::Time::now();
  is_cloud_3_time_out = false;
}
...
void PointcloudConcatenate::subCallbackCloudIn4(sensor_msgs::PointCloud2 msg) {
  ...
  cloud_in4_received_time = ros::Time::now();
  is_cloud_4_time_out = false;
}
...
void PointcloudConcatenate::handleParams() {
  ...
  param_name = node_name_ + "/POINT_CLOUD_TIME_OUT";
  if (!ros::param::get(param_name, param_point_cloud_time_out_)) {
    param_point_cloud_time_out_ = 10;
    ROSPARAM_WARN(param_name, param_point_cloud_time_out_);
  }
...
void PointcloudConcatenate::update() {
  ...
  if (pub_cloud_out.getNumSubscribers() > 0 && param_clouds_ >= 1) {
    ...
    if (param_clouds_ >= 1 && success && cloud_in1_received) {
      ros::Duration elapsed  = ros::Time::now() - cloud_in1_received_time;
      ...
      if (!cloud_in1_received_recent) {
        if (elapsed_sec > param_point_cloud_time_out_){
          ROS_WARN_STREAM_THROTTLE(1,"Time out from Cloud 1 data. Stopping ");
          is_cloud_1_time_out = true;
          }
    ...
    if (param_clouds_ >= 2 && success && cloud_in2_received) {
      ros::Duration elapsed  = ros::Time::now() - cloud_in2_received_time;
      double elapsed_sec = elapsed.toSec(); //local variable

      // Warn if cloud was not received since last update
      if (!cloud_in2_received_recent) {
        if (elapsed_sec > param_point_cloud_time_out_){
          ROS_WARN_STREAM_THROTTLE(1,"Time out from Cloud 2 data. Stopping ");
          is_cloud_2_time_out = true;
          }
    ...
    if (param_clouds_ >= 3 && success && cloud_in3_received) {
      ros::Duration elapsed  = ros::Time::now() - cloud_in3_received_time;
      double elapsed_sec = elapsed.toSec(); //local variable

      // Warn if cloud was not received since last update
      if (!cloud_in3_received_recent) {
        if (elapsed_sec > param_point_cloud_time_out_){
          ROS_WARN_STREAM_THROTTLE(1,"Time out from Cloud 3 data. Stopping ");
          is_cloud_3_time_out = true;
    ...
    if (param_clouds_ >= 4 && success && cloud_in4_received) {
      ros::Duration elapsed  = ros::Time::now() - cloud_in4_received_time;
      double elapsed_sec = elapsed.toSec(); //local variable

      // Warn if cloud was not received since last update
      if (!cloud_in4_received_recent) {
        if (elapsed_sec > param_point_cloud_time_out_){
          ROS_WARN_STREAM_THROTTLE(1,"Time out from Cloud 4 data. Stopping ");
          is_cloud_4_time_out = true;
          }
    ...
    // Publish the concatenated pointcloud only when all sensors are good.
    // ROS_WARN_STREAM(" " << success << " " << is_cloud_1_time_out << " " << is_cloud_2_time_out << " "  << is_cloud_3_time_out << " "  << is_cloud_4_time_out << " " );
    if (success and (!is_cloud_1_time_out) and (!is_cloud_2_time_out) and (!is_cloud_3_time_out) and (!is_cloud_3_time_out)) {
