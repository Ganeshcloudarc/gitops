...
class StanlyControl:
    def __init__(self):
        ...
        if od_enable:
            ...
            ack_pub_topic = '/vehicle/cmd_drive_safe1'

        self.ack_pub_safe = rospy.Publisher(ack_pub_topic,AckermannDrive, queue_size=10)
        self.vehicle_pose_pub = rospy.Publisher('/vehicle/pose1',PoseStamped, queue_size=10)
        self.vehicle_tarpose_pub = rospy.Publisher('/vehicle/target_pose1',PoseStamped, queue_size=10)
        ...
        self.control_gain_k = 0.01  # control gain
    ...
    def stanley_control(self, target_idx, error_front_axle):
        ...
        print("heading error: ",theta_e)
        # theta_d corrects the cross track error
        theta_d = arctan2(self.control_gain_k * error_front_axle, self.robot['vel'])
        print("cross track error: ",theta_d)
    ...
    def main_loop(self):
        ...
        r = rospy.Rate(5)
        target_idx,_ = self.calc_nearest_ind(self.robot,0)
        while not rospy.is_shutdown(): 
            print("----------------------------------------------------------")
            ## distance fun test
            ...
            target_idx, error_front_axle = self.calc_nearest_ind(self.robot,target_idx)
            ...
            print('point ',self.path[target_idx])  
            self.target_point_msg.pose.position.x  = self.path[target_idx][0]
            self.target_point_msg.pose.position.y = self.path[target_idx][1]
            self.target_point_msg.pose.orientation = Quaternion(*quaternion_from_euler(0,0, self.path[target_idx][2]))

            
            self.vehicle_tarpose_pub.publish(self.target_point_msg)

            delta  = self.stanley_control(target_idx, error_front_axle)
