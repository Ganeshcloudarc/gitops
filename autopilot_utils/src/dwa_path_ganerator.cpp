...
namespace autopilot_utils
{
    ...
    DwaPathGenerator::DwaPathGenerator(double wheel_base, double steering_angle_max, double steering_inc, double max_path_length, double path_resolution) {
    ...
    void DwaPathGenerator::update_params(double wheel_base, double steering_angle_max, double steering_inc, double max_path_length, double path_resolution) {
        this->wheel_base = wheel_base;
        this->steering_angle_max = steering_angle_max;
        this->steering_inc = steering_inc;
        this->max_path_length = max_path_length;
    ...
    std::vector<std::vector<std::vector<double>>> DwaPathGenerator::generate_paths(std::vector<double> robot_pose = {0, 0, 0}) {
        if (robot_pose == std::vector<double>{0, 0, 0} && init) {
            return path_from_origin;
        } else {
            std::vector<std::vector<std::vector<double>>> paths;
            for (double i = steering_angle_max; i > -steering_angle_max - steering_inc; i -= steering_inc) {
                if (i == 0) {
                    i = 0.01;
                }
    ...
    std::vector<std::vector<std::vector<double>>> DwaPathGenerator::get_path_from_origin() {
        return path_from_origin;
    }

    void DwaPathGenerator::transform_path(std::vector<double> robot_pose) {
        // Transform all the origin points by robot pose
    }

    visualization_msgs::MarkerArray DwaPathGenerator::get_dwa_paths_marker_array(std::vector<std::vector<std::vector<double>>> dwa_paths, std::string target_frame) {
      ...
      visualization_msgs::MarkerArray DwaPathGenerator::get_dwa_paths_marker_array(std::vector<std::vector<double>> dwa_paths, std::string target_frame) {
        visualization_msgs::MarkerArray marker_arr_msg;
