...
bool raytraceLineLookGap(costmap_2d::Costmap2D &costmap, double x1, double y1,
  ...
  if (costmap.worldToMap(x1, y1, mx1, my1) and
      costmap.worldToMap(x2, y2, mx2, my2)) {
    double dist = 0;
    unsigned char cost;
    ...
    // cout<<x1<<" "<<y1<<" "<<endl;
    // cout<<x2<<" "<<y2<<" "<<endl;
    while (x != mx2 or y != my2) { // cout<<"x : " <<x <<" y : "<<y1;
...
std::pair<bool, unsigned char> raytraceLineCost(costmap_2d::Costmap2D &costmap,
  ...
  if (costmap.worldToMap(x1, y1, mx1, my1) and
    ...
    while (x != mx2 or y != my2) { // cout<<"x : " <<x <<" y : "<<y1;
      ...
      // cout<<"x :" <<x<<" y:"<<y<<endl;
      // cout<<"cost :"<<static_cast<unsigned int>(cost)<<endl;
      if (cost >= cost_th) { // cout<<"COST is not ZERO"<<endl;
...
std::pair<bool, unsigned char> raytraceLineCost(costmap_2d::Costmap2D &costmap,
  ...
  if (costmap.worldToMap(x1, y1, mx1, my1) and
    ...
    while (x != mx2 or y != my2) { // cout<<"x : " <<x <<" y : "<<y1;
      ...
      // cout<<"x :" <<x<<" y:"<<y<<endl;
      // cout<<"cost :"<<static_cast<unsigned int>(cost)<<endl;
      if (cost != 0) { // cout<<"COST is not ZERO"<<endl;
        return std::make_pair(true, cost);
      }
    }
...
OccupencyGridManager::OccupencyGridManager(ros::NodeHandle nh,
                                           const string topic,
                                           bool subscribe_to_updates = false)
    : occupency_map_received(false) {
  occupency_sub = nh.subscribe<nav_msgs::OccupancyGrid>(
      topic, 1, &OccupencyGridManager::OccupancyGridCallback, this);
  if (subscribe_to_updates) {

    occupency_updaes_sub = nh.subscribe<map_msgs::OccupancyGridUpdate>(
        topic + "updates", 1,
        &OccupencyGridManager::OccupancyGridUpdateCallback, this);
  }
}

void OccupencyGridManager::OccupancyGridCallback(
    const nav_msgs::OccupancyGrid::ConstPtr &occupancy_grid_msg) {
  // costmap_2d.setDefaultValue(0);
  costmap_2d.resizeMap(occupancy_grid_msg->info.width,
...
void OccupencyGridManager::OccupancyGridUpdateCallback(
    const map_msgs::OccupancyGridUpdate::ConstPtr &update_msg) {
...
unsigned char OccupencyGridManager::get_line_cost_map(unsigned int x1,
                                                      unsigned int y1,
                                                      unsigned int x2,
                                                      unsigned int y2) {
  unsigned char cost;
  ...
  // cout<<x1<<" "<<y1<<" "<<endl;
  // cout<<x2<<" "<<y2<<" "<<endl;
  while (x != x2 or y != y2) { // cout<<"x : " <<x <<" y : "<<y1;
    int double_error = 2 * error;
    if (double_error > -dy) {
      error -= dy;
      x += sx;
...
std::pair<bool, unsigned char>
OccupencyGridManager::get_line_cost_world(double x1, double y1, double x2,
                                          double y2) {
  unsigned int mx1, my1, mx2, my2;
  if (costmap_2d.worldToMap(x1, y1, mx1, my1) and
      costmap_2d.worldToMap(x2, y2, mx2, my2)) {
    unsigned char cost = get_line_cost_map(mx1, my1, mx2, my2);
