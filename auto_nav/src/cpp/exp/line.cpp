...
public:
    Pose2D(double x, double y, double yaw=0, double
circum_radius=std::numeric_limits<double>::infinity(), int turn_info=0) {
        this->x = x;
        this->y = y;
        this->yaw = yaw;
    ...
    std::string toString() {
        ...
        oss << "x: " << x << " y: " << y << " yaw: " << yaw * 180 / M_PI << "
circum_radius: " << circum_radius << " turn_info: " << turn_info; return
oss.str();
    }
...
0.0); if (dotProduct >= 0) { return true; } else { return false;
        }
    }
    ...
    bool headingCheck(const Pose2D& other, double th=M_PI/2) {
        ...
        // double dotProduct = std::inner_product(v1.begin(), v1.end(),
v2.begin(), 0.0); double angle = getAngle(v1, v2); if (abs(angle) < M_PI/2) {
            return true;
        }
        return false;
    }
...
private:
    double getAngle(const std::vector<double>& v1, const std::vector<double>&
v2) { double dotProduct = std::inner_product(v1.begin(), v1.end(), v2.begin(),
0.0); double magnitudeV1 = sqrt(std::inner_product(v1.begin(), v1.end(),
v1.begin(), 0.0)); double magnitudeV2 = sqrt(std::inner_product(v2.begin(),
v2.end(), v2.begin(), 0.0)); double cosTheta = dotProduct / (magnitudeV1 *
magnitudeV2); return acos(cosTheta);
    }
...
public:
  ...
  // std::vector<int> inlier(std::vector<std::vector<double>> points, double
  // offset) {
  //     double dino = sqrt(m * m + (-1 * -1));
  ...
  // std::vector<int> inliers_cross_product(std::vector<std::vector<double>>
  // cloud_points, double offset) {
  //     std::vector<double> p1 = {0, m * 0 + c};
  //     std::vector<double> p2 = {1, m * 1 + c};
  //     std::vector<int> points_ind;
  //     for (int i = 0; i < cloud_points.size(); i++) {
  //         std::vector<double> p = cloud_points[i];
  //         double dir_perpendicular_dis = (p2[0] - p1[0]) * (p1[1] - p[1]) -
  //         (p1[0] - p[0]) * (p2[1] - p1[1]); dir_perpendicular_dis /=
  //         sqrt(pow(p2[0] - p1[0], 2) + pow(p2[1] - p1[1], 2)); if
  //         (abs(dir_perpendicular_dis) <= offset) {
