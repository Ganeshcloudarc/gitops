...
AutoNavCore::AutoNavCore()
    : tf2_listener(tf2_buffer), odom_data_received(false),
      global_traj_data_received(false), curr_scan_data_received(false),
      curr_local_cloud_data_received(false), in_turn_status(false) {
  ros::NodeHandle private_nh("~");
  loadParams(private_nh);
  diag_status.name = ros::this_node::getName();
  dwa_path_gen.update_params(dwa_wheel_base, dwa_steering_agle_lim,
                             dwa_constant_speed, dwa_path_len,
                             dwa_path_resolution);

  if (use_costmap_for_dwa and enable_dwa)
    costmap_ros_ = new costmap_2d::Costmap2DROS("/costmap", tf2_buffer);
  // ROS_DEBUG_STREAM(costmap_ros->getGlobalFrameID());
  scan_sub = private_nh.subscribe<sensor_msgs::LaserScan>(
      "/laser_scan", 1, &AutoNavCore::scanCallback, this);
  local_pointcloud_sub = private_nh.subscribe<sensor_msgs::PointCloud2>(
      "/local_cloud_map", 1, &AutoNavCore::localPointcloudsCallback, this);
  trajectory_sub = private_nh.subscribe<autopilot_msgs::Trajectory>(
      "/global_gps_trajectory", 1, &AutoNavCore::trajectoryCallback, this);
  odometry_sub = private_nh.subscribe<nav_msgs::Odometry>(
      "/vehicle/odom", 1, &AutoNavCore::odomCallback, this);
  local_traj_pub = private_nh.advertise<autopilot_msgs::Trajectory>(
      "/local_gps_trajectory", 1);
  local_path_pub = private_nh.advertise<nav_msgs::Path>("/local_trajectory", 1);
  path_percent_publisher =
      private_nh.advertise<std_msgs::Float32>("/osp_path_percentage", 10);
  lanes_marker_pub =
  ...
  if (pub_debug_topics) {
    ...
    front_pose_pub =
        private_nh.advertise<geometry_msgs::PoseStamped>("front_pose", 1);
    map_point_cloud_pub =
        private_nh.advertise<sensor_msgs::PointCloud2>("map_point_cloud", 1);
    left_liners_cloud_pub =
        private_nh.advertise<sensor_msgs::PointCloud2>("left_inliers", 1);
    right_inliers_cloud_pub =
        private_nh.advertise<sensor_msgs::PointCloud2>("right_inliers", 1);
    dwa_marker_pub = private_nh.advertise<visualization_msgs::MarkerArray>(
        "dwa_paths", 1, true);
    dwa_collsion_free_marker_pub =
        private_nh.advertise<visualization_msgs::MarkerArray>(
            "dwa_paths_collsion_free", 1, true);
    dwa_best_traj_marker_pub =
        private_nh.advertise<visualization_msgs::MarkerArray>("dwa_best_traj",
                                                              1, true);

    ransac_samples_pub = private_nh.advertise<visualization_msgs::Marker>(
        "ransac_samples", 1, true);
    vibration_path_pub =
        private_nh.advertise<nav_msgs::Path>("vibration_path", 1);
  }

  // double max_steer = 30;
  // DwaPathGenerator dwa_path_gen(2.5, max_steer, 1, 10, 0.1);
  // std::vector<std::vector<std::vector<double>>> paths =
  // dwa_path_gen.generate_paths({0, 0, 0});
...
void AutoNavCore::loadParams(ros::NodeHandle private_nh) {
  ...
  private_nh.param<int>("moving_avg_filter_window_size",
                        moving_avg_filter_window_size, 10);
  ...
  private_nh.param<float>("orchard_details/tree_width_tolerace",
                          tree_width_tolerance, 0.3);

  private_nh.param<float>("turnings/radius_to_check_turn", radius_to_check_turn,
                          15);
  private_nh.param<float>("turnings/minimum_turn_radius", minimum_turn_radius,
                          7);
  ...
  private_nh.param<bool>("use_global_path_for_ransac",
                         use_global_path_for_ransac, false);
  private_nh.param<bool>("reset_costmap_once", reset_costmap_once, false);
  private_nh.param<bool>("enable_center_line_collision_checking",
                         enable_center_line_collision_checking, false);

  // DWA related
  private_nh.param<bool>("dwa/enable", enable_dwa, false);

  private_nh.param<bool>("dwa/use_costmap", use_costmap_for_dwa, true);
  ...
  private_nh.param<float>("dwa/steering_angle_increment",
                          dwa_steering_angle_increment, 1);
...
void AutoNavCore::main_loop(ros::NodeHandle private_nh) {
  ...
  ros::Rate rate(5);
  while (ros::ok()) {
    if (curr_local_cloud_data_received and curr_scan_data_received and
        global_traj_data_received and odom_data_received) {
  ...
  while (ros::ok()) {
    ...
    // checking for timeouts from sensors and odometry
    // Odom, localcloud, laserscan
    ...
    if (close_index == -1) {
      auto val = traj_helper.find_closest_idx_with_dist_ang_thr(
          curr_robot_pose, row_spacing, M_PI / 2);

      if (val.first) {
        close_index = val.second;
      } else {
      ...
      close_index = traj_helper.find_close_pose_after_index(curr_robot_pose,
                                                            close_index, 10);
    }
    ...
    // path percentage
    path_percent_val.data =
        (global_traj.points[close_index].accumulated_distance_m /
         global_traj.points.back().accumulated_distance_m) *
        100;
    ...
    int forward_path_idx =
        traj_helper.next_point_within_dist(close_index, row_spacing);
    ROS_DEBUG_STREAM("forward_path_idx : " << forward_path_idx);
    if (pub_debug_topics) {
      geometry_msgs::PoseStamped pst;
      pst.header.frame_id = "map";
      pst.pose =
          traj_helper.get_trajectory_point_by_index(forward_path_idx).pose;
    ...
    int p1_index =
        traj_helper.next_point_within_dist(close_index, forward_point_dis);

    int p2_index =
        traj_helper.next_point_within_dist(p1_index, forward_point_dis);

    int p3_index =
        traj_helper.next_point_within_dist(p2_index, forward_point_dis);
    vector<double> p1 = {
        traj_helper.get_trajectory_point_by_index(p1_index).pose.position.x,
        traj_helper.get_trajectory_point_by_index(p1_index).pose.position.y};
    vector<double> p2 = {
        traj_helper.get_trajectory_point_by_index(p2_index).pose.position.x,
        traj_helper.get_trajectory_point_by_index(p2_index).pose.position.y};
    vector<double> p3 = {
        traj_helper.get_trajectory_point_by_index(p3_index).pose.position.x,
        traj_helper.get_trajectory_point_by_index(p3_index).pose.position.y};
    float circum_radius = circumRadius(p1, p2, p3);
    ROS_DEBUG_STREAM("circum_radius : " << circum_radius);
    auto close_pose =
        traj_helper.get_trajectory_point_by_index(close_index).pose;
    Vector3d start_point_v(cos(tf::getYaw(close_pose.orientation)),
                           sin(tf::getYaw(close_pose.orientation)), 0);
    /* Auto Turnings
    ...
    if (abs(circum_radius) < radius_to_check_turn or in_turn_status == true) {
      ...
      if (turn_start_index == -1 and turn_end_index == -1) {
        ...
        for (int i = turn_start_index; i < traj_helper.getLength(); i++) {
          ...
          Vector3d end_point_v(traj_pose.position.x - close_pose.position.x,
                               traj_pose.position.y - close_pose.position.y, 0);
      ...
      for (int i = close_index; i < loop_end_index; i++) {
        ...
        turn_traj.points.push_back(
            traj_helper.get_trajectory_point_by_index(i));
        geometry_msgs::PoseStamped pst;
        pst.header.frame_id = "map";
   ...
   if ((global_turn_detected or  in_turn_status) and !row_end_point.empty())
    {
        ...
        if (moving_avg_center_line.valid())
        {
          ...
          Eigen::Vector3d end_point_to_robot
   {row_end_point[0]-curr_robot_pose.position.x,
   row_end_point[1]-curr_robot_pose.position.y , 0}; Eigen::Vector3d unit_vector
   = unit_vect(moving_avg_center_line.getHeadingAngle()); double dot_pro =
   end_point_to_robot.dot(unit_vector); cout<<"dot_pro"<<dot_pro<<endl; if
   (dot_pro > 0)
          {
            // Both opposuite direction , vehicle is still in the row send the
   avg_line points


          vector<double> start_point_center_lane =
   moving_avg_center_line.intersct_point_to_line(std::vector<double>{curr_robot_pose.position.x,
   curr_robot_pose.position.y}); vector<double> forward_point_center_lane =
   moving_avg_center_line.intersct_point_to_line(std::vector<double>{row_end_point[0],
   row_end_point[1]}); cout<<"before"<<endl; local_trajectory_msg =
   twoPointsToTrajectory(start_point_center_lane, forward_point_center_lane,
   0.1,"map", speed); // speed 1 cout<<"afer"<<endl;
      ...
      Eigen::Vector3d dir_vect =  {p1[0]- row_end_point[0], p1[1]-
   row_end_point[1], 0};


      double crossProduct = unit_vector.x() * dir_vect.y() - unit_vector.y() *
   dir_vect.x();
      ...
      double q1[] = {row_end_point[0]+x_scaled, row_end_point[1]+y_scaled,
   row_end_point[2]+ M_PI };
      ...
      while (mix_x <  length) {
        ...
        // end_point_to_robot = {q[0]-curr_robot_pose.position.x,
   q[1]-curr_robot_pose.position.y , 0};
        // double dist = sqrt(pow(curr_robot_pose.position.x - q[0], 2) +
        //               pow(curr_robot_pose.position.y - q[1],2));
        // Eigen::Vector3d  v_dubins = unit_vect(q[2]);

        // Eigen::Vector3d  v_robot =
   unit_vect(tf::getYaw(curr_robot_pose.orientation));
        ...
        local_trajectory_msg.points.push_back(DubinsPointToTrajectoryPoint(q,
   mix_x, speed));
      ...
      end_point_to_robot =
   {local_trajectory_msg.points.back().pose.position.x-curr_robot_pose.position.x,
   local_trajectory_msg.points.back().pose.position.y-curr_robot_pose.position.y
   , 0}; double dis =
   distanceBetweenPoses(curr_robot_pose,local_trajectory_msg.points.back().pose
   );

      Eigen::Vector3d  unit_vector =
   unit_vect(tf::getYaw(local_trajectory_msg.points.back().pose.orientation));
     ...
     Auto Turnings end*/
      ...
      // scan_projector.transformLaserScanToPointCloud("map",*curr_local_scan,
      // cloud, tf2_buffer);

      std::vector<Eigen::Vector3d> local_map_points;
      local_map_points =
          kiss_icp_ros::utils::PointCloud2ToEigen(local_pointcloud);
      Header header;
      header.frame_id = "map";
      // if (pub_debug_topics)
      //  {
      //   const auto eigen_cloud = EigenToPointCloud2(local_map_points,
      //   header); map_point_cloud_pub.publish(*std::move(eigen_cloud));}
      Eigen::Vector3d L1, L2;
      if (use_global_path_for_ransac) {
        L1 = {traj_helper.get_trajectory_point_by_index(close_index)
                  .pose.position.x,
              traj_helper.get_trajectory_point_by_index(close_index)
                  .pose.position.y,
              0.0};
        L2 = {traj_helper.get_trajectory_point_by_index(forward_path_idx)
                  .pose.position.x,
              traj_helper.get_trajectory_point_by_index(forward_path_idx)
                  .pose.position.y,
              0.0};
      } else {
        L1 = {curr_robot_pose.position.x, curr_robot_pose.position.y, 0.0};
        L2 = {curr_robot_pose.position.x +
                  cos(tf::getYaw(curr_robot_pose.orientation)),
              curr_robot_pose.position.y +
                  sin(tf::getYaw(curr_robot_pose.orientation)),
              0.0};
      }
      std::vector<Eigen::Vector3d> left_close_points, right_close_points,
          all_close_points;
      ...
      for (int i = 0; i < ransac_max_iterations; i++) { // left points
        ...
        inliers_left = distancesToLine(left_close_points, offset, line_point1,
                                       line_point2);
        ...
        Eigen::Vector3d line_point3(line_point1.x() + new_x,
                                    line_point1.y() + new_y, 0.0);
        Eigen::Vector3d line_point4(line_point2.x() + new_x,
                                    line_point2.y() + new_y, 0.0);
        vector<int> inliers_right;
        inliers_right = distancesToLine(right_close_points, offset, line_point3,
                                        line_point4);
        // inlilers_right_count = inliers_right.size();

        if (inliers_left.size() > max_inlilers_left and
            inliers_right.size() > max_inlilers_right) {
        ...
        inliers_right_ = distancesToLine(right_close_points, offset,
                                         line_point1_, line_point2_);
        // inlilers_left_count = inliers_left.size();
        ...
        Eigen::Vector3d line_point3_(line_point1_.x() + new_x_,
                                     line_point1_.y() + new_y_, 0.0);
        Eigen::Vector3d line_point4_(line_point2_.x() + new_x_,
                                     line_point2_.y() + new_y_, 0.0);
        vector<int> inliers_left_;
        inliers_left_ = distancesToLine(left_close_points, offset, line_point3_,
                                        line_point4_);
        // inlilers_right_count = inliers_right.size();
        // cout<<"inliers_left_  :"<<inliers_left_.size()<<endl;

        if (inliers_left_.size() > max_inlilers_left and
            inliers_right_.size() > max_inlilers_right) {
      ...
      const auto left_eigen_cloud =
          EigenToPointCloud2(left_inliers_final, header);
      if (pub_debug_topics)
        left_liners_cloud_pub.publish(*std::move(left_eigen_cloud));

      const auto right_eigen_cloud =
          EigenToPointCloud2(right_inliers_final, header);
      if (pub_debug_topics)
        right_inliers_cloud_pub.publish(*std::move(right_eigen_cloud));
      sensor_msgs::PointCloud2 right_eigen_cloud_base_link,
          left_eigen_cloud_base_link;
      geometry_msgs::TransformStamped transform_to_local_frame;
      try {
        transform_to_local_frame = tf2_buffer.lookupTransform(
            "base_link", right_eigen_cloud->header.frame_id, ros::Time(0));
        tf2::doTransform(*right_eigen_cloud, right_eigen_cloud_base_link,
                         transform_to_local_frame);
        tf2::doTransform(*left_eigen_cloud, left_eigen_cloud_base_link,
                         transform_to_local_frame);
      ...
      right_eigen_base_link =
          kiss_icp_ros::utils::PointCloud2ToEigen(right_eigen_cloud_base_link);

      pair<double, double> right_coeffs =
          leastSquareMethod(right_eigen_base_link);
      ROS_WARN_STREAM("right params - slope : " << right_coeffs.first
                                                << ", intercept : "
                                                << right_coeffs.second);
      std::vector<Eigen::Vector3d> left_eigen_base_link;
      left_eigen_base_link =
          kiss_icp_ros::utils::PointCloud2ToEigen(left_eigen_cloud_base_link);
      pair<double, double> left_coeffs =
          leastSquareMethod(left_eigen_base_link);
      ROS_WARN_STREAM("left params - slope : " << left_coeffs.first
                                               << ", intercept : "
                                               << left_coeffs.second);

      Line left_lane((left_coeffs.first + right_coeffs.first) / 2,
                     left_coeffs.second);
      Line right_lane((left_coeffs.first + right_coeffs.first) / 2,
                      right_coeffs.second);
      Line center_lane((left_coeffs.first + right_coeffs.first) / 2,
                       (left_coeffs.second + right_coeffs.second) / 2);
      ...
      point1_center.x = -val;
      point1_center.y =
          center_lane.getSlope() * -val + center_lane.getConstant();
      ...
      geometry_msgs::Point point1_center_map, point2_center_map,
          point1_left_map, point2_left_map, point1_right_map, point2_right_map;
      try {
        transform_to_base_link =
            tf2_buffer.lookupTransform("map", "base_link", ros::Time(0));
        tf2::doTransform(point1_center, point1_center_map,
                         transform_to_base_link);
        tf2::doTransform(point2_center, point2_center_map,
                         transform_to_base_link);
        ...
        tf2::doTransform(point1_right, point1_right_map,
                         transform_to_base_link);
        tf2::doTransform(point2_right, point2_right_map,
                         transform_to_base_link);
      } catch (tf2::TransformException &ex) {
        ROS_WARN("%s", ex.what());
        ROS_ERROR("Could not transform");
      ...
      double slope = (point2_center_map.y - point1_center_map.y) /
                     (point2_center_map.x - point1_center_map.x);
      ...
      if (slope_list.size() > moving_avg_filter_window_size and
          intercept_list.size() > moving_avg_filter_window_size) {
        // slope_list.pop_back();
      ...
      cout << "slope sum : " << slope_sum
           << " intercept sum :" << intercept_sum;
      ...
      geometry_msgs::Point robot_point, robot_forward_point,
          start_point_center_lane, forward_point_center_lane;
      robot_point.x = curr_robot_pose.position.x;
      robot_point.y = curr_robot_pose.position.y;
      robot_forward_point.x =
          curr_robot_pose.position.x +
          cos(tf::getYaw(curr_robot_pose.orientation)) * (local_traj_length);
      robot_forward_point.y =
          curr_robot_pose.position.y +
          sin(tf::getYaw(curr_robot_pose.orientation)) * (local_traj_length);
      start_point_center_lane =
          moving_avg_center_lane.intersct_point_to_line(robot_point);
      forward_point_center_lane =
          moving_avg_center_lane.intersct_point_to_line(robot_forward_point);
      ...
      double line_heading =
          atan2(forward_point_center_lane.y - start_point_center_lane.y,
                forward_point_center_lane.x - start_point_center_lane.x);
      moving_avg_center_line.update_line(moving_avg_slope,
                                         moving_avg_intercept);
      ...
      if (use_costmap_for_dwa) {
        ...
        // costmap_->raytraceLine(start_point_center_lane.x,
        // start_point_center_lane.y, forward_point_center_lane.x,
        // forward_point_center_lane.y);

        // Checking collsions on center line on costmap
        unsigned char cost_th = 254;
        pair<bool, unsigned char> center_collision = raytraceLineCost(
            *costmap_, start_point_center_lane.x, start_point_center_lane.y,
            forward_point_center_lane.x, forward_point_center_lane.y, cost_th);
        ROS_DEBUG_STREAM("first"
                         << center_collision.first << " second : "
                         << static_cast<unsigned int>(center_collision.second));
        if (center_collision.first) {
          if (center_collision.second == 0) {
          ...
          } else if (center_collision.second >= cost_th) {
            ROS_ERROR("collions found on moving_avg_center_lane");
            ROS_DEBUG_STREAM("enable_center_line_collision_checking : "
                             << enable_center_line_collision_checking);
            if (enable_center_line_collision_checking) {
              diag_status.summary(
                  ERROR,
                  "Collision detected on center line stoping the vehicle");
              diag_status.add("Line_Collision_status", true);
              autopilot_msgs::Trajectory line_traj;
              line_traj.header.frame_id = "map";
              nav_msgs::Path line_path;
              line_path.header.frame_id = "map";
              for (int i = 0; i < local_traj_length / dwa_path_resolution;
                   i++) {
                autopilot_msgs::TrajectoryPoint traj_point;
                traj_point.pose.position.x =
                    start_point_center_lane.x +
                ...
                traj_point.pose.position.y =
                    ...
                    sin(line_heading) * i * dwa_path_resolution;
                traj_point.pose.orientation =
                    get_quaternion_from_yaw(line_heading);
            ...
            ROS_WARN("Collision detected on center line, but still not stoping "
                     "the vehicle");
            diag_status.add("Line_Collision_status", true);
            diag_status.add("line collsiion cost", center_collision.second);
          }

        } else {
          ROS_ERROR("Could not find cost to line");
          diag_status.add("Line_Collision_status",
                          "Could not find cost to line");
        }

        // Finding the row end detection using left lane and right lanes
        Eigen::Vector3d c1(start_point_center_lane.x, start_point_center_lane.y,
                           0);
        Eigen::Vector3d c2(forward_point_center_lane.x,
                           forward_point_center_lane.y, 0);
        ...
        // if (costmap.worldToMap(x1, y1, mx1, my1) and costmap.worldToMap(x2,
        // y2, mx2, my2))
        // {

        // }
        double lx_free, ly_free, llx_free, lly_free, rx_free, ry_free, rrx_free,
            rry_free;

        if (raytraceLineLookGap(*costmap_, l1.x(), l1.y(), l2.x(), l2.y(),
                                lx_free, ly_free, 1)) {
          ROS_ERROR_STREAM("Free left end points detected x :"
                           << lx_free << " y:" << ly_free);
        } else {
          ROS_INFO_STREAM("No left free end points detected");
        }
        marker_arr.markers.push_back(xy_to_marker(lx_free, ly_free, 10));
        if (raytraceLineLookGap(*costmap_, ll1.x(), ll1.y(), ll2.x(), ll2.y(),
                                llx_free, lly_free, 1)) {
          ROS_ERROR_STREAM("Free left end points detected x :"
                           << llx_free << " y:" << lly_free);
        } else {
        ...
        if (raytraceLineLookGap(*costmap_, r1.x(), r1.y(), r2.x(), r2.y(),
                                rx_free, ry_free, 1)) {
          ROS_ERROR_STREAM("Free right end points detected x :"
                           << rx_free << " y:" << ry_free);
        } else {
        ...
        if (raytraceLineLookGap(*costmap_, rr1.x(), rr1.y(), rr2.x(), rr2.y(),
                                rrx_free, rry_free, 1)) {
          ROS_ERROR_STREAM("Free right end points detected x :"
                           << rrx_free << " y:" << rry_free);
        } else {
        ...
        // Dubins curves connecting mid points
        double q0[] = {(lx_free + rx_free) / 2, (ly_free + ry_free) / 2,
                       line_heading};
        vect = vect.normalized() * (2 * row_spacing);
        x_scaled = vect.x() * cos(-M_PI / 2) - vect.y() * sin(-M_PI / 2);
        y_scaled = vect.x() * sin(-M_PI / 2) + vect.y() * cos(-M_PI / 2);
        double q1[] = {(lx_free + rx_free) / 2 + x_scaled,
                       (ly_free + ry_free) / 2 + y_scaled, line_heading + M_PI};

        row_end_point = {(lx_free + rx_free) / 2, (ly_free + ry_free) / 2,
                         line_heading};
        ...
        for (int i = 0; i < dwa_paths.size(); i++) {
          ...
          for (int j = 0; j < dwa_paths[i].size(); j++) {
            unsigned int mx, my;
            // ROS_DEBUG_STREAM("world to map :"<<"x : "<<dwa_paths[i][j][0]
            // <<"y :" <<dwa_paths[i][j][1]<<"cost_val :"
            // <<occ_manager->costmap_2d.worldToMap(dwa_paths[i][j][0],
            // dwa_paths[i][j][1], mx, my));

            if (costmap_->worldToMap(dwa_paths[i][j][0], dwa_paths[i][j][1], mx,
                                     my)) {

              // ROS_DEBUG_STREAM("cell cost : "<<static_cast<unsigned
              // int>(occ_manager->costmap_2d.getCost(mx, my)));
        ...
        ROS_DEBUG_STREAM(
            "collision free patths len: " << collision_free_path_ids.size());
        ROS_DEBUG_STREAM("reset_costmap_once status : " << reset_costmap_once);
        if (collision_free_path_ids.size() == 0) {
          ROS_ERROR(
              "Could not found collision free DWA paths, Stoping the vehicle");
          diag_status.summary(
              ERROR,
              "Could not found collision free DWA paths, Stoping the vehicle");
          diag_status.add("reset_costmap", reset_costmap_once);
          // costmap_ = costmap_ros_->getCostmap();
          if (reset_costmap_once == true) {
            costmap_ros_->resetLayers();
            //  reset_costmap = false;
          }
          autopilot_msgs::Trajectory traj;
          traj.header.frame_id = "map";
          local_traj_pub.publish(traj);
          publish_diagnostics();
          loop_rate.sleep();
          ros::spinOnce();
          continue;
        ...
        // robot_forward_point,start_point_center_lane,
        // forward_point_center_lane ; double moving_avg_center_lane_heading =
        // atan(forward_point_center_lane.y - start_point_center_lane.y /
        // forward_point_center_lane.x - start_point_center_lane.x);
        vector<double> dist_list, angle_diff_list, dist_list_norm,
            angle_diff_norm, total_norm;
        ...
        for (int i = 0; i < dist_list.size(); i++) {
          ...
          total_norm.push_back(dist_list[i] / dist_sum +
                               angle_diff_list[i] / angle_diff_sum);
        }

        final_dwa_path = collision_free_paths[min_cost_index];
      }
        ...
        double min_dis = 1000;
        for (int i = 0; i < dwa_paths.size(); i++) {
          vector<double> point = dwa_paths[i].back();
          double dis = moving_avg_center_lane.distance_to_point(point);
          double angle_diff = line_heading - point[2];
          dist_list.push_back(dis);
          angle_diff_list.push_back(angle_diff);
        ...
        final_dwa_path = dwa_paths[min_cost_index];
      }
      ...
      for (int i = 0; i < final_dwa_path.size(); i++) {
        autopilot_msgs::TrajectoryPoint traj_point;
        traj_point.pose.position.x = final_dwa_path[i][0];
        traj_point.pose.position.y = final_dwa_path[i][1];
        traj_point.pose.orientation =
            get_quaternion_from_yaw(final_dwa_path[i][2]);
        traj_point.longitudinal_velocity_mps =
            traj_helper.get_trajectory_point_by_index(close_index)
                .longitudinal_velocity_mps;
        if (i == 0)
          ...
          traj_point.accumulated_distance_m =
              dwa_traj.points.back().accumulated_distance_m +
              distanceBetweenPoses(dwa_traj.points.back().pose,
                                   traj_point.pose);
...
void AutoNavCore::localPointcloudsCallback(
    const sensor_msgs::PointCloud2::ConstPtr &data) {
  local_pointcloud = data;
  curr_local_cloud_data_received = true;
  last_localcloud_time = ros::Time::now();
}
...
void AutoNavCore::publish_empty_traj() {
  ...
  local_path_pub.publish(path);
}

void AutoNavCore::trajectoryCallback(
    const autopilot_msgs::Trajectory::ConstPtr &trajectory) {
...
int main(int argc, char **argv) {
  ...
  // ros::init_options::AnonymousName); ros::init(argc, argv,
  // "autonav_core_node", ros::init_options::AnonymousName); ros::init(argc,
  // argv, "autonav_core_node", ros::init_options::AnonymousName);
  ros::init(argc, argv, "autonav_core_node");
