...
def convert_path(path, to_frame):
    ...
    if tfBuffer is None or listener is None:
        _init_tf()
    try:
        # trans = tfBuffer.lookup_transform(to_frame, path.header.frame_id, rospy.Time.now(), rospy.Duration(3.0))
        ...
        # trans = tfBuffer.lookup_transform(to_frame, path.header.frame_id, rospy.Time.now())
        trans = tfBuffer.lookup_transform(to_frame, path.header.frame_id, rospy.Time.now(), rospy.Duration(1.0))
...
if __name__ == "__main__":
    ...
    ## test plan for path
    pub1 = rospy.Publisher("base_link_plan", Path, queue_size=1, latch=True)
    pub2 = rospy.Publisher("map_plan", Path, queue_size=1, latch=True)
    print("current robot pose", current_robot_pose())
    ...
    print("before", path.poses[1])
    pub1.publish(path)

    path2 = convert_path(path, "base_link")
    # path2 = path
    print("after")
    print(path2.poses[1])
    pub2.publish(path2)
