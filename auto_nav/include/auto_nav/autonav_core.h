...
private:
  void localPointcloudsCallback(
      const sensor_msgs::PointCloud2::ConstPtr &local_scan);
  void scanCallback(const sensor_msgs::LaserScan::ConstPtr &local_scan);
  void
  trajectoryCallback(const autopilot_msgs::Trajectory::ConstPtr &trajectory);
  ...
  ros::Publisher local_traj_pub, local_path_pub, center_line_pub, left_line_pub,
      right_line_pub, path_percent_publisher, front_pose_pub;
  ros::Publisher map_point_cloud_pub, left_liners_cloud_pub,
      right_inliers_cloud_pub;
  ros::Publisher lanes_marker_pub, dwa_marker_pub, dwa_collsion_free_marker_pub,
      ransac_samples_pub;
  ...
  bool odom_data_received, global_traj_data_received, curr_scan_data_received,
  ...
  // Parameters
  bool enable_dwa;
  bool use_costmap_for_dwa, enable_moving_avg_filter, pub_debug_topics,
      use_previous_line, mission_continue, use_global_path_for_ransac,
  ...
  vector<double> next_row_start_point, next_left_row_start_point,
      next_right_row_start_point;
