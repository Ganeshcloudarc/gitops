...
class ObstacleStopPlanner:
    def __init__(self):
        ...
        self.radius_to_search = vehicle_data.dimensions.overall_width / 2 + self.radial_off_set_to_vehicle_width
        ...
        self.local_traj_publisher = rospy.Publisher('local_gps_trajectory', Trajectory, queue_size=1)
        self.collision_points_publisher = rospy.Publisher('collision_points', PointCloud2, queue_size=1)
        self.velocity_marker_publisher = rospy.Publisher('collision_velocity_marker', MarkerArray, queue_size=1)

        self.obstacleBoundingBox = rospy.Publisher("obstacle_bounding_box", BoundingBox,queue_size=1)
        self.foot_prin_pub = rospy.Publisher("/obs/foot_print", PolygonStamped, queue_size=1)
        ...
        while not rospy.is_shutdown():
            ...
            if self.traj_in and robot_pose:
                rospy.loginfo(" global path and robot_pose  are received")
                break
            else:
                rospy.logwarn("waiting for global path or robot_pose ")
                rate.sleep()
        count = 0
        rate = rospy.Rate(20)
        robot_pose = current_robot_pose("map", self.robot_base_frame)
                ...
                if self.traj_in.points[ind].accumulated_distance_m - \
                    ...
                    try:
                        for i in range(0,self.size-1):
                            ...
                            other_coordinates = self.GetCoordinates(pos_x,pos_y,dimX/2,dimY/2, get_yaw(robot_pose.orientation))
                            ...
                            self.foot_prin_pub.publish(polygon_st)
                        
                            if (obs_dist or obs_dist1  or obs_dist2 or obs_dist3 or obs_dist4) < self.radius_to_search:
                                rospy.logerr('Obstacle found........')
                                self.from_bb = 1
                                self.pub_mark(i)
...
#                         rospy.logwarn("could not query KD tree", str(error))
                ...
                if (self.from_bb): #or laser_obs(laser_obs and len(list(collision_points[0])) > 0): #or len(list(collision_points[0])) > 0:
...
#                 self.publish_points(collision_points_list)
    ...
    def GetCoordinates(self,x, y,dimensionsX,dimensionsY, yaw = 3.14):
        cos_th = math.cos(yaw)
        sin_th = math.sin(yaw)
        all_coordinates = []
        ...
        all_coordinates.extend([[co1X,co1Y],[co3X,co3Y],[co2X,co2Y],[co4X,co4Y]])
    ...
    def pub_mark(self,id):
        obs_bb_msg = BoundingBox()
        # obs_bb_msg.header.frame_id = "map"
        obs_bb_msg = self.box_list[id]
        self.obstacleBoundingBox.publish(obs_bb_msg)
...
if __name__ == "__main__":
    rospy.init_node('obstacle_stop_planner_lidar_node')
