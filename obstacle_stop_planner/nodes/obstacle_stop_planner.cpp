...
pair<bool, std::string> ObstacleStopPlanner::do_initial_sensor_check() {
    ...
    bool sensor_status = is_lidar_sensor_healthy && is_zed_sensor_healthy && is_global_path_ok;
...
void ObstacleStopPlanner::main_loop() {
  ...
  while (!ros::isShuttingDown()) {
    ...
    double odom_time_difference = loop_start_time - odom_data_in_time;

    // Debug logging
    ROS_INFO("Loop start time: %f", loop_start_time);
    ROS_INFO("Last odom update time: %f", odom_data_in_time);
    ROS_INFO("Time difference: %f", odom_time_difference);

    if (odom_time_difference > _TIME_OUT_FROM_ODOM) {
        ROS_WARN_THROTTLE(2, "No Update on Odom (Robot Position)");
        diagnostics_publisher.summary(ERROR, "No update on odom (robot position)");
        diagnostics_publisher.add("Loop start time", loop_start_time);
    ...
    // Bypass Mode
    int prev_processed_ind = _close_idx;
    ...
    if (by_pass_dist > 0) {
      ...
      end_by_pass_index = ind; 
        ...
        for (int ind = start_by_pass_index; ind < end_by_pass_index+1; ind++) {
      ...
      diagnostics_publisher.summary(ERROR, "BYPASS_MODE");
    ...
    if (use_obs_v1) {
      ...
      try {
        ...
        c++ version. The Python version takes the 2d data as its library takes care of it
        whereas c++ version library takes 3d data so from the map frame if a obstacle is 
        at the edges the kd-tree query is ignoring the obstacles as it has the z data.
        */
        else {
    ...
    if (robot_level_collision_check_enable) {
      ...
      obs_found_sensor = "";
      radius_to_search_near_robot = (data_vehicle_obj.overall_width + width_offset) / 2;
      ...
      if (use_obs_v1) {
          ...
          for (const auto &circle : circles_array) {
              ...
              // Check whether the points have NAN or inf because kd-tree will crash if these values are passed so skipping it
              // || !std::isfinite(query_point.z)
              if (!std::isfinite(query_point.x) || !std::isfinite(query_point.y)) {
                  ROS_ERROR("Circle query points (%f, %f) have non-finite values. Skipping it.", query_point.x, query_point.y);
                  continue;
              }

              try {
                  // Search for obstacles using radius search && !pointIdxRadiusSearch.empty()
                  ...
                  ROS_ERROR("Exception caught in use_obs_v1 circle array logic: %s", e.what());
              }
          }
      }

      // If any obstacle found in V1 then this v2 won't get executed (Achieved by the flag variable obs_found_robot_collision)
      if (use_pcl_boxes && !obs_found_robot_collision && !bboxes.boxes.empty()) {
      ...
      // If any obstacle found in V1 or v2 then this won't get executed (Achieved by the flag variable obs_found_robot_collision)
      if (use_zed_detections && !obs_found_robot_collision) {
          // Run obstacle check using Zed detections if enabled
          ROS_INFO_ONCE("Running Zed");

          for (const auto &circle : circles_array) {
              if (find_close_object_zed(zed_objects, circle) < radius_to_search_near_robot) {
      ...
      if (obs_found_robot_collision) {
          // If obstacle is found, stop immediately and publish diagnostics
          ...
          ROS_WARN("RLC : Immediate stop as an obstacle is found near the vehicle");
          ROS_INFO_THROTTLE(10,"Collision Points Published");
          diagnostics_publisher.summary(ERROR, "Obstacle in front");
          diagnostics_publisher.add("Obstacle Detected length offset", length_offset);
          ...
          // If no obstacles found, continue normal operation
          ROS_INFO("RLC : No Obstructions present in the proximity of the vehicle\n");
      }
  ...
  // ------------------------- End of Robot Level Collision-----------------------------------------------
    ...
    for (; global_index < _traj_end_index; global_index++) {
      ...
      if (_traj_in.points[global_index].accumulated_distance_m -
        ...
        if (use_zed_detections && !obstacle_encountered) {
          ...
          if (zed_objects != NULL) {
            vector<double> point_obj = {path_pose.position.x,
                                        path_pose.position.y};
            close_obj_from_zed = find_close_object_zed(zed_objects, point_obj);

            if (close_obj_from_zed < _radius_to_search) {
              ROS_WARN_THROTTLE(
                  10,
                  "Obstacle Detected on the Camera: %ld",
                  static_cast<long int>(close_obj_from_zed));
              obstacle_found = true;
            ...
            ROS_DEBUG(
              ...
              try {
                ...
                if (kdtree.radiusSearch(search_point, _radius_to_search,
                  ...
                  new_collision_points = pointIdxRadiusSearch; // For Publishing the collision points
                  obstacle_found = true;
                  obstacle_encountered = true;
    ...
    if (obstacle_encountered) {
      ...
      if (dis_to_obstacle < (_stop_line_buffer + _base_to_front)) {
        ...
        diagnostics_publisher.add("Obstacle Detected Sensor ",obstacle_found_sensor_main_logic);
        publish(diagnostics_publisher);
        ...
        ROS_INFO("The obstacle is at a distance greater than the stopping distance\n");
        ...
        if (velocity_speed_profile_enable) {
          try {
            ...
            // Speed sent to zero to the trajectory from collision index to stop index
            ...
            // slow_stop_index
            for (int index = stop_index; index > slow_stop_index; --index) {
              tmp_dist =
                  abs(_traj_in.points[stop_index].accumulated_distance_m -
...
// camera
double ObstacleStopPlanner::find_close_object_zed(
    zed_interfaces::ObjectsStamped::ConstPtr objects, vector<double> point) {
  // Store the distances to ZED-detected objects
  vector<float> zed_obs_dis_data;
  ...
  */
  ...
  for (auto object : objects->objects) {
    ...
    dis = hypot(point[0] - object.position[0], point[1] - object.position[1]);
    ...
    */
    if (dis < _radius_to_search) {
      zed_obs_dis_data.push_back(dis);
    } else {
      // Check distances to corners for redundancy check
      for (auto corners : object.bounding_box_3d.corners) {
        dis = hypot(point[0] - corners.kp[0], point[1] - corners.kp[1]);
        zed_obs_dis_data.push_back(dis);
      }
    }
  }
  try {
    // Check if there are valid distances in the vector
    if (zed_obs_dis_data.size() > 0) {
      // Find the minimum distance index
      zed_obs_dis_data_min = std::distance(
          zed_obs_dis_data.begin(),
          std::min_element(zed_obs_dis_data.begin(), zed_obs_dis_data.end()));
      // Return the minimum distance
      return zed_obs_dis_data[zed_obs_dis_data_min];
    }
  } catch (const std::exception &e) {
    // Using ROS logging macros for error messages
    ROS_ERROR_STREAM("Exception from find_close_object_zed: " << e.what());
    // Return infinity if an exception occurs
    return std::numeric_limits<double>::infinity();
  }

  // Add a default return statement (or handle this case based on your logic)
  return std::numeric_limits<double>::infinity();
}
